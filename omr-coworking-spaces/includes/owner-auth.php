<?php
/**
 * Space Owner Authentication Helpers
...
function ownerLogin(string $email): bool {
    ...
    $stmt = $conn->prepare("SELECT id, email, full_name, status FROM space_owners WHERE email = ?");
    ...
    if (!$owner) {
        ...
        $stmt = $conn->prepare("INSERT INTO space_owners (full_name, email, phone, password_hash, status) VALUES (?, ?, ?, ?, 'pending')");
