...
myomr_module_require_routed('HOSTELS_ADMIN_ROUTED', '/superadmin/hostels/index.php');
require_once __DIR__ . '/_urls.php';
/**
 * Admin Dashboard - Hostels & PGs Module
 */

$title = 'Hostels & PGs Dashboard';

require_once __DIR__ . '/../../core/omr-connect.php';
global $conn;

// Fetch summary stats
$total_properties = $conn->query("SELECT COUNT(*) FROM hostels_pgs")->fetch_row()[0];
$active_properties = $conn->query("SELECT COUNT(*) FROM hostels_pgs WHERE status = 'active'")->fetch_row()[0];
$pending_owners = $conn->query("SELECT COUNT(*) FROM property_owners WHERE status = 'pending'")->fetch_row()[0];
$total_inquiries = $conn->query("SELECT COUNT(*) FROM property_inquiries")->fetch_row()[0];
$total_owners = $conn->query("SELECT COUNT(*) FROM property_owners")->fetch_row()[0];
      ...
      <div class="row mb-4">
        <div class="col-md-4">
          <div class="card shadow-sm">
            <div class="card-body">
              <h5 class="card-title"><i class="fas fa-home me-2"></i>Recent Properties</h5>
              <?php if ($recent_properties->num_rows > 0): ?>
                <ul class="list-unstyled mb-0">
                  <?php while ($prop = $recent_properties->fetch_assoc()): ?>
                    <li class="py-2 border-bottom">
                      <strong><?php echo htmlspecialchars($prop['property_name']); ?></strong>
                      <br><small class="text-muted"><?php echo date('M d, Y', strtotime($prop['created_at'])); ?></small>
                    </li>
                  <?php endwhile; ?>
                </ul>
              <?php else: ?>
        ...
        <div class="col-md-3 mb-3">
          <div class="card shadow-sm">
            <div class="card-body text-center">
              ...
              <a href="manage-properties.php" class="btn btn-primary">View All</a>
            </div>
          </div>
        </div>
