...
$stmt = $conn->prepare('SELECT slno, industry_name, address, contact, industry_type, locality, verified, about, services, careers_url FROM omr_industries WHERE slno = ?');
...
$canonical = 'https://myomr.in/industries/' . $slug;
$canonical_url = $canonical; $page_title = htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . ' | Industry on OMR | MyOMR'; $page_description = htmlspecialchars($name . ' - Industry on Chennai OMR. Address: ' . $address, ENT_QUOTES, 'UTF-8'); $og_title = $page_title; $og_description = $page_description; $og_url = $canonical; $og_image = 'https://myomr.in/My-OMR-Logo.png';
?>
<!DOCTYPE html>
<html lang="en">
<head>
<?php $breadcrumbs = [
  ['https://myomr.in/','Home'],
  ['https://myomr.in/industries','Industries'],
  [$canonical, $name]
]; ?>
<?php include '../components/meta.php'; ?>
<?php $ga_custom_params = ['listing_type' => 'industry', 'locality' => $row['locality'] ?? '', 'industry_type' => $row['industry_type'] ?? '']; include '../components/analytics.php'; ?>
...
<div id="main-content" class="container maxw-1280">
  ...
  if ($detectedLocality !== '') {
    ...
    $rel = $conn->prepare('SELECT slno, industry_name, address FROM omr_industries WHERE slno <> ? AND address LIKE ? ORDER BY industry_name ASC LIMIT 6');
...
$stmt = $conn->prepare('SELECT slno, industry_name, address, contact, industry_type FROM omr_industries WHERE slno = ?');
$stmt->bind_param('i', $id);
$stmt->execute();
$res = $stmt->get_result();
...
<html lang="en">
<head>
<?php $breadcrumbs = [ ['https://myomr.in/','Home'], ['https://myomr.in/industries','Industries'], [$canonical, $name] ]; ?>
<?php include '../components/meta.php'; ?>
<?php $ga_custom_params = ['listing_type' => 'industry']; include '../components/analytics.php'; ?>
<?php include '../components/head-resources.php'; ?>
