...
$page_description = 'Explore ' . $locality . ' on Old Mahabalipuram Road (OMR): IT companies, banks, hospitals, restaurants, schools, parks and more.';
$og_title = $page_title;
$og_description = $page_description;
$og_url = 'https://myomr.in/omr-listings/locality/perungudi.php';
...
<div class="container" style="max-width:1280px;">
  ...
  <div class="row mb-3">
    <?php
      $cards = [
        ['IT Companies', '/it-companies?locality=' . urlencode($locality)],
        ['Banks', '/omr-listings/banks.php'],
        ['Hospitals', '/omr-listings/hospitals.php'],
        ['Restaurants', '/omr-listings/restaurants.php'],
        ['Schools', '/omr-listings/schools.php'],
        ['Parks', '/omr-listings/parks.php'],
      ];
      foreach ($cards as $c) {
  ...
  <?php
    $stmt = $conn->prepare('SELECT slno, company_name, address FROM omr_it_companies WHERE (locality = ? OR address LIKE ?) ORDER BY company_name ASC LIMIT 12');
    $like = '%' . $locality . '%';
  ...
  $breadcrumbs = [
    ...
    '@type' => 'BreadcrumbList',
    'itemListElement' => [
      [ '@type' => 'ListItem', 'position' => 1, 'item' => ['@id' => 'https://myomr.in/', 'name' => 'Home'] ],
      [ '@type' => 'ListItem', 'position' => 2, 'item' => ['@id' => $og_url, 'name' => $locality] ]
    ]
  ];
