...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['email'])) {
    ...
    header('Location: /superadmin/newsletter-subscribers.php?done=1');
    exit;
}
...
$activeNav = '/superadmin/newsletter-subscribers.php';
...
<div class="container-fluid py-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-0"><i class="fas fa-envelope me-2"></i><?= htmlspecialchars($page_title) ?></h1>
    <div class="d-flex gap-2">
      <a href="/superadmin/newsletter-subscribers.php?export=csv<?= $filter_status ? '&status=' . urlencode($filter_status) : '' ?><?= $search ? '&q=' . urlencode($search) : '' ?>"
         class="btn btn-sm btn-success fw-semibold">
        <i class="fas fa-download me-1"></i>Export CSV
      </a>
      <a href="/superadmin/index.php" class="btn btn-sm btn-outline-secondary">
  ...
  <form method="get" class="row g-2 mb-3">
    ...
    <div class="col-md-2">
      <a href="/superadmin/newsletter-subscribers.php" class="btn btn-outline-secondary w-100">Clear</a>
    </div>
  </form>
    ...
    <ul class="pagination pagination-sm">
      <?php for ($p = 1; $p <= $total_pages; $p++): ?>
        <li class="page-item <?= $p === $cur_page ? 'active' : '' ?>">
          <a class="page-link"
             href="/superadmin/newsletter-subscribers.php?page=<?= $p ?><?= $filter_status ? '&status=' . urlencode($filter_status) : '' ?><?= $search ? '&q=' . urlencode($search) : '' ?>">
            <?= $p ?>
          </a>
        </li>
      <?php endfor; ?>
