<?php
/**
 * MyOMR — 404 Page Content
 * Included by core/serve-404.php and 404.php.
 * Expects: $missed_url, $referrer. Analytics must be loaded before this.
 */
  ...
  (function poll() {
    if (typeof gtag === 'function') {
      gtag('event', 'page_not_found', {
        'page_location': <?= json_encode('https://' . ($_SERVER['HTTP_HOST'] ?? 'myomr.in') . ($missed_url ?? $_SERVER['REQUEST_URI'] ?? '/'), _GA_JSON_FLAGS) ?>,
...
<body>
<?php require_once (defined('ROOT_PATH') ? ROOT_PATH : (__DIR__ . '/..')) . '/components/main-nav.php'; ?>
...
<div class="err-hero">
  <div class="container">
    ...
    <p class="badge-url"><?= $missed_url ?? htmlspecialchars($_SERVER['REQUEST_URI'] ?? '/', ENT_QUOTES, 'UTF-8') ?></p>
...
<?php require_once (defined('ROOT_PATH') ? ROOT_PATH : (__DIR__ . '/..')) . '/components/footer.php'; ?>
</body>
</html>
