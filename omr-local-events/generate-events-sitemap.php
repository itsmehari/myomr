...
$evRoot = 'https://myomr.in' . MYOMR_EVENTS_HUB_PATH;
$urls = [];
// Listing page (canonical short hub)
$urls[] = $evRoot . '/';
// Date intent hubs
$urls[] = $evRoot . '/today';
$urls[] = $evRoot . '/weekend';
...
try {
  ...
  if ($conn && !$conn->connect_error) {
    ...
    while ($row = $res->fetch_assoc()) {
      ...
      $urls[] = $evRoot . '/locality/' . urlencode($slug);
    }
    ...
    while ($row = $res->fetch_assoc()) {
      ...
      $urls[] = $evRoot . '/venue/' . urlencode($vslug);
    }
  }
} catch (Throwable $e) {
